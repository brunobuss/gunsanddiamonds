...
static int almp3_decode_frame_mp3(ALMP3_MP3 *mp3, char *inbuf, char *outbuf, int *size_done, int *inread) {
  ...
  *inread = 0;
  ...
  if (ret == MP3_NEED_MORE) {
    ...
    data_left = mp3->data_len - ((char *)mp3->data - mp3->data_cursor);
    do {
      char *p = inbuf;
      if (data_left <= 0)
        break;
      ...
      if (data_left < chunk_size) {
        ret = decodeMP3(&(mp3->mpeg), p, data_left, outbuf, 8192, size_done);
        p += data_left;
        *inread += data_left;
        data_left = 0;
      }
      else {
        ret = decodeMP3(&(mp3->mpeg), p, chunk_size, outbuf, 8192, size_done);
