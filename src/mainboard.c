...
void NovoJogo(void)
{
	...
	/* Iniciando os indicadores graficos */
	show_mouse(NULL);
			...
			if(clique == 2) /* Hora de, tentar, remover as joias */
			{
				...
				switch(GetDirection(coord_clique[0][0], coord_clique[0][1], coord_clique[1][0], coord_clique[1][1]))
				{
					case 0:
						if(!(matriz_grade[coord_clique[0][0]][coord_clique[0][1]].troca_cima ||
						     matriz_grade[coord_clique[1][0]][coord_clique[1][1]].troca_baixo)) clique =0;
					break;
					case 1:
						if(!(matriz_grade[coord_clique[0][0]][coord_clique[0][1]].troca_direita ||
						     matriz_grade[coord_clique[1][0]][coord_clique[1][1]].troca_esquerda)) clique =0;
					break;
					case 2:
						if(!(matriz_grade[coord_clique[0][0]][coord_clique[0][1]].troca_baixo ||
						     matriz_grade[coord_clique[1][0]][coord_clique[1][1]].troca_cima)) clique =0;
					break;
					case 3:
						if(!(matriz_grade[coord_clique[0][0]][coord_clique[0][1]].troca_esquerda ||
						     matriz_grade[coord_clique[1][0]][coord_clique[1][1]].troca_direita)) clique =0;
					break;
				}
				...
				while(!free_to_go)
				{
					while(!free_to_go)
					{
						...
						show_mouse(NULL);
						if(PreencheLinha0(matriz_grade, Joias, buffer)) /* Se nao ha mais espacos em branco, free_to_go = 1 */
						{
							free_to_go = 0;
							BufferToScreen(buffer, screen);
						}
		...
		for (coluna=0; coluna<10 ;coluna++)
		{
			while(1)
			{
				...
				if((linha < 2) && (coluna < 2)) /* Nao e necessario fazer checagem aqui, pois nao a possibilidade de ter 3 iguais... sao os 4 do canto superior esquerdo*/
					break;
				else if((linha < 2) && !((matriz_grade[linha][coluna].joia == matriz_grade[linha][coluna-1].joia) && (matriz_grade[linha][coluna].joia == matriz_grade[linha][coluna-2].joia))) /* Checagem para as 2 primeiras linhas... logo nao precisamos fazer verificacao vertical, so horizontal */
					break;
				else if((coluna < 2) && !((matriz_grade[linha][coluna].joia == matriz_grade[linha-1][coluna].joia) && (matriz_grade[linha][coluna].joia == matriz_grade[linha-2][coluna].joia))) /* Checagem para as 2 primeiras colunas... logo nao precisamos fazer verificacao horizontal, so vertical */
					break;
...
int GetDirection(int quad1_linha, int quad1_coluna, int quad2_linha, int quad2_coluna)
{
	...
	if(quad1_linha > quad2_linha)        return 0; /* Quad 2 esta acima do quad1 */
	else if(quad1_coluna < quad2_coluna) return 1; /* Quad 2 esta a direita do quad1 */
	else if(quad1_linha < quad2_linha)   return 2; /* Quad 2 esta abaixo do quad1 */
	else if(quad1_coluna > quad2_coluna) return 3; /* Quad 2 esta a esquerda do quad1 */
...
int PreCacheMoves(JOIA matriz_grade[10][10])
{
	...
	for(i =0; i<10;i++)
	{
		for(j=0;j<10;j++)
		{
			...
			if(matriz_grade[linha][coluna].joia == 0) continue;
			...
			for(j=0;j<10;j++)
			{
				if(vai_cair[i][j])
				{
					...
					if(k == 15)
					{
						...
						if(i == 0) matriz_grade[i][j].joia = 0;
					}
				}
			}
	}
...
int SearchAndDestroy(JOIA matriz_grade[10][10], BITMAP *buffer, BITMAP* fundo)
{
	...
	for(i=0;i<10;i++)
		for(j=0;j<10;j++) sd[i][j] = 0;

	for(i=0;i<10;i++)
		for(j=0;j<10;j++) if(VerificaMovimento(matriz_grade, i, j)) sd[i][j] = 1;
...
int LoadButtons(BITMAP *buttons[3])
{
	...
	if(!buttons[0] || !buttons[1] || !buttons[2]) return 1;
	...
	for(i=0;i<10;i++)
		for(j=0;j<10;j++)
			if(matriz_grade[i][j].troca_cima ||
			   ...
			   matriz_grade[i][j].troca_baixo) return 1;
...
int PassouDeFase(int fase, int pontos)
{
	...
	{
		case 1:
			if(pontos >= FASE_2_PONTOS) return 1;
			break;
		case 2:
			if(pontos >= FASE_3_PONTOS) return 1;
			break;
		case 3:
			if(pontos >= FASE_4_PONTOS) return 1;
			break;
		case 4: if(pontos >= FASE_5_PONTOS) return 1;
			break;
		case 5:
			if(pontos >= FASE_6_PONTOS) return 1;
			break;
	}
...
void DeloadAll(BITMAP *Joias[8], BITMAP *fundo, BITMAP *buffer, BITMAP *buttons[3], BITMAP *numeros[10], BITMAP *mira)
{
	...
	for(i=0;i<3;i++)  destroy_bitmap(buttons[i]);
	for(i=0;i<8;i++) destroy_bitmap(Joias[i]);
	for(i=0;i<10;i++) destroy_bitmap(numeros[i]);
